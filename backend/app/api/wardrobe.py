...
def delete_wardrobe_item(
    ...
    affected_outfit_ids = [
        oid for (oid,) in db.query(OutfitItem.outfit_id)
        .filter(OutfitItem.wardrobe_item_id == item.id)
        .all()
    ]
    ...
    # Remove outfit links to this item and drop outfits that lose a top or bottom.
    if affected_outfit_ids:
        db.query(OutfitItem).filter(OutfitItem.wardrobe_item_id == item.id).delete(synchronize_session=False)
        outfits = db.query(Outfit).filter(Outfit.id.in_(affected_outfit_ids)).all()
        for outf in outfits:
            cats = [
                cat for (cat,) in db.query(WardrobeItem.category)
                .join(OutfitItem, OutfitItem.wardrobe_item_id == WardrobeItem.id)
                .filter(OutfitItem.outfit_id == outf.id)
                .all()
            ]
            if not (any(c == "top" for c in cats) and any(c == "bottom" for c in cats)):
                db.query(OutfitItem).filter(OutfitItem.outfit_id == outf.id).delete(synchronize_session=False)
                db.delete(outf)
