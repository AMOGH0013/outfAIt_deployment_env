...
def _primary_color(value: str | None) -> str:
    return normalize_color_name(value)
...
def compute_user_style_profile(db: Session, user_id: UUID) -> UserStyleProfile:
    ...
    for outfit_id, category, item_type, color, palette, embedding in rows:
        ...
        if cat == "top":
            ...
            for p in pal[:5]:
                bump(top_color_scores, _primary_color(str(p)), w * 0.25)
        elif cat == "bottom":
            bump(bottom_type_scores, itype, w)
            bump(bottom_color_scores, col, w)
            for p in pal[:5]:
                bump(bottom_color_scores, _primary_color(str(p)), w * 0.25)
