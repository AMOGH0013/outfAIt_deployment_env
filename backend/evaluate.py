...
def run_color_extraction_tests():
    ...
    log(f"  • color_extraction.py → extract_dominant_colors(): If KMeans is still run")
    log(f"    in BGR, cluster centers can drift from perceptual color boundaries.")
    log(f"    Fix: run KMeans in LAB and use LAB centers directly.")
...
def run_mask_quality_tests():
    ...
    log(f"  WHERE TO LOOK:")
    log(f"  • sam_segmentation.py → segment_clothing(): For person-worn photos,")
    log(f"    foreground points should stay tight (35%-65% width, 40%-60% height)")
    log(f"    and include explicit background points at edges/corners.")
    ...
    log(f"  • _fallback_threshold_mask(): loose thresholds on gray backgrounds can")
    log(f"    capture the background. Prefer GrabCut where possible.")
